...
| Method | Description |
| --- | --- |
| `selectTrack(index: number): void` | Load track `index` into the player via `loadTrack(...)`; updates the active-row UI, resets `currentChapterIndex` to `-1` (clearing any chapter highlight), and reveals that track's nested `.wp-chapters` block. No-op if out of range. |
| `seekToChapter(trackIndex: number, time: number): void` | Same track: `seekTo(time)` (and `play()` if paused). Cross-track: selects `trackIndex` and seeks once that track's `onLoad` fires. No-op if `trackIndex` is out of range. |
...
| `getTracks(): WaveformPlaylistTrack[]` | The parsed track objects (`element`, `index`, `url`, `title`, `artist`, `artwork`, `album`, `duration`, `waveform`, `chapters[]`, `markers[]`). Chapters are sorted by time. |
| `destroy(): void` | Removes the keydown listener, cancels any pending chapter seek, destroys the player, removes only the nodes and layout classes the playlist added (your own classes stay), un-hides the `[data-track]` elements in place, clears `data-waveform-playlist-initialized`, and nulls `player` / `listElement` / `tracks`. The container can then be initialised again. |
...
`seekToChapter` to a different track loads that track first and seeks only once the player's `onLoad` fires **for that track** — so a deep link like `seekToChapter(1, 60)` straight after construction lands correctly. There is only ever one pending seek: a load error, selecting another track, or `destroy()` cancels it, so it can never land on a different track. With `preload="none"` the duration isn't known until playback starts, so the playlist starts playback and seeks when the metadata arrives. (All 1.8.0+; earlier versions keyed off `waveformplayer:ready`, which the core emits once after construction, and could seek before the track had a duration.)
</Aside>
...
Keys pressed with **Ctrl, Cmd or Alt** held pass through untouched (1.8.0+), so browser shortcuts like Cmd/Ctrl+P (print) and Ctrl/Alt+digit (switch tab) keep working. Shift is allowed.
...
The playlist dispatches **no custom DOM events of its own** — it has no emit path. Instead it drives itself off the core player's callbacks. Any of these you pass as options still run, **after** the playlist's handling (1.8.0+):
...
| player `onTimeUpdate(current, total)` | Re-highlights the active chapter row (`.wp-active` + `aria-current`; chapters sharing a start time highlight the first), updates the hero/grid time readout, and — once the duration is known — warns about chapters that start after the track ends. |
...
| player `onLoad(player)` | Runs a pending cross-track chapter seek if this is the track it was waiting for. |
| player `onError` | Cancels a pending chapter seek. |
| player `onNextTrack` / `onPreviousTrack` | The lock-screen / Media Session skip buttons call `nextTrack()` / `previousTrack()`. |
...
| Class | Element |
| --- | --- |
| `.waveform-playlist` | The container (base styles), added on init. Layout modifiers: `.wp-minimal`, `.wp-hero-layout`, `.wp-grid-layout`, plus `.wp-density-compact` (`density: 'compact'`), `.wp-cover-top` (`coverPosition: 'top'`) and `.wp-no-artist` (`showArtist: false`). |
...
Call `destroy()` when removing a playlist (e.g. in a SPA route change). It removes the keydown listener, destroys the inner player, removes the generated UI, and leaves your original `[data-track]` markup in place — so the same container can be initialised again with `new WaveformPlaylist(el, …)` or `WaveformPlaylist.init()`. (Before 1.8.0 it wiped the container first, so the tracks were lost and the React/Vue/Svelte wrappers rebuilt an empty playlist on a prop change.)
