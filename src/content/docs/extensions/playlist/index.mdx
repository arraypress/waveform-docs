...
Here's a real, playable playlist — pick any track and it loads into the shared player (auto-advance is on):

<PlaylistDemo />

That's nothing but a `[data-waveform-playlist]` container wrapping one `[data-track]` per row — the exact markup covered below.
