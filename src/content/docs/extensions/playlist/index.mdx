...
`WaveformPlaylist` turns a list of tracks into a navigable playlist, album or chaptered episode — driving a single embedded [WaveformPlayer](/player/options/) underneath. It parses declarative `[data-track]` / `[data-chapter]` markup, renders the list UI, wires up keyboard navigation, and loads each track into the player on demand.
...
Install the playlist alongside the core player (the player is a peer dependency, `^1.24.5` from 1.8.0 — previously `^1.7.2`, which the playlist had long outgrown).
...
The original `[data-track]` markup is hidden (`display: none`) during init and restored in place on [`destroy()`](/extensions/playlist/api/#public-methods), with any inline `display` it had put back.
...
| `data-waveform` | No | Pre-computed peaks for this track (1.8.0+): a JSON array of peaks, or a `.json` peaks URL (e.g. from [`waveform-gen`](/extensions/gen/)) passed through for the player to fetch. Skips decoding the audio just to draw it. A malformed JSON array logs a warning and the track falls back to decoding. |

<Aside type="caution" title="data-markers must be valid JSON">
`data-markers` must be a JSON array. Malformed JSON (or anything that isn't an array) logs a `[WaveformPlaylist] Invalid markers …` warning and that track simply gets no explicit markers; entries without a numeric `time` are dropped. The rest of the playlist still renders, but validate it server-side so the markers you meant actually show.
</Aside>
...
| `data-time` | No | Timestamp as `M:SS`, `MM:SS`, `H:MM:SS` (1.8.0+ — earlier versions read `1:05:30` as 1 second), or bare seconds (e.g. `90`). A malformed value reads as `0:00`. Omitted, the chapter is placed at `0:00` with a console warning. |
