...
| `repeat` | `'off' \| 'all' \| 'one'` | `'off'` | Initial repeat mode. `'all'` loops the queue; `'one'` loops the current track. Applies with `showRepeat: false` too (1.12.0+); an unknown value falls back to `'off'`. |
...
| `persist` | `boolean` | `true` | Save queue/index/position/playing to `sessionStorage`, and volume/mute/favorites to `localStorage`. Also gates the collapsed-state save. `false` writes nothing to either store — volume, mute and favorites included (1.12.0+; earlier versions saved those three regardless). |
...
| `showTime` | `boolean` | `true` | Render the current / total time display. `false` omits it in both layouts (1.12.0+; earlier versions ignored the option and always showed the time). |
...
| `barRadius` | `number \| null` | `null` | **1.12.0+** Rounded bar caps in px; `0` = square (forwarded even though it's falsy). `null` keeps the player's own default. |
...
| `waveformGradient` | `'vertical' \| 'horizontal' \| 'diagonal'` | `'vertical'` | Axis for gradient stop arrays in `waveformColor`/`progressColor`. Forwarded to the player's [`waveformGradient`](/player/options/). |
...
| `crossOrigin` | `'anonymous' \| 'use-credentials' \| null` | `null` | CORS mode forwarded to the embedded `<audio>`. Only passed when truthy. `null` leaves it unset (native `<audio>` behavior — avoids breaking CDN media without CORS headers). |
...
| `onQueueChange` | `(queue, currentIndex) => void` | A track is added (`addToQueue`, 1.12.0+), removed or cleared. **Not** fired by `play()`-driven insertions. |
| `onVolumeChange` | `(volume) => void` | `setVolume` runs (and the unmute path). Not fired by the mute-only branch. |
| `onShuffleChange` | `(shuffle) => void` | Shuffle is turned on or off. |
