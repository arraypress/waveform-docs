...
import BarDemo from '~/components/BarDemo.astro';
...
`@arraypress/waveform-bar` 1.12.0+ declares a peer dependency of `@arraypress/waveform-player@^1.8.0` (earlier versions: `^1.7.2`) — external mode, `setProgress()`/`setPlayingState()` and `barRadius` all need 1.8.0. Keep the player at or above that range. The bar ships **no `.d.ts`** — typed wrappers (e.g. `waveform-bar-react` / `-astro`) hand-declare the config type.
</Aside>
