...
- Each event fires **at most once per track**, tracked per player. The set re-arms on `ended`, so a replay can fire them all again, and when the player switches to a different URL (`loadTrack()` or `load()`), so the new track gets its own events (1.0.2+).
