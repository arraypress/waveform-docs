...
It renders no DOM, ships no CSS, sets no cookies, and collects no PII. Drop it on a page, call `init()`, and it auto-discovers every player.
...
<CardGrid>
  ...
  <Card title="Seeks ignored" icon="random">
    A forward jump of 5s or more only counts when real time kept pace with it (a throttled background tab); anything beyond that is a seek and dropped — scrubbing never inflates your numbers.
  </Card>
  ...
  <Card title="Survives unload" icon="rocket">
    Terminal events go out with `fetch` + `keepalive` — or `navigator.sendBeacon` for a same-origin endpoint — so they land even as the page navigates away.
  </Card>

</CardGrid>

Because it's headless, the tracker is easiest to *see* with a live readout. The player below is wired to a tracker whose `handler` renders each captured event straight into the panel (no server) — the `listen` threshold is lowered to 5s so it fires quickly. Press play, let it run, pause, scrub — a jump ahead isn't credited, so scrubbing won't hurry the `listen` event along:
...
| `delta >= 5s`, within the real time that passed | Yes | A background tab whose updates were throttled, reporting minutes of playback in one jump (1.1.0+) |
| `delta >= 5s`, beyond it | No | Treated as a **seek** (see `SEEK_THRESHOLD` below) and dropped |

The stretch between the last `timeupdate` and the track's `ended` event is credited by the same rule (1.1.0+), so a track that finishes in a hidden tab isn't short-changed.

The result is an honest "seconds of content heard" number. Pausing stops crediting and clears the delta baseline, but it does **not** reset the accumulated total — resuming continues where you left off. The accumulated total and the fired-event set reset when the track `ended` fires, which re-arms every event for a replay, and when the player moves to a different URL, so a track swapped in mid-play starts from zero instead of inheriting the previous track's time (1.0.2+).
...
`SEEK_THRESHOLD` is the forward `currentTime` jump (in seconds) below which a delta is always credited as playback. It is a **class-level constant, not a config option** — it lives on the class, so `window.WaveformTracker.SEEK_THRESHOLD` (the exported singleton) is `undefined`, and there is no runtime knob to change it. The player fires `timeupdate` far more often than once every 5 seconds, so foreground playback never trips it.

A larger jump isn't automatically a seek (1.1.0+). Browsers throttle hidden tabs, so the player's updates can stall while the audio plays on, then report minutes of playback at once. The tracker compares the jump with the wall-clock time since the previous update and credits it when it is no more than `elapsed × playbackRate × 1.5 + 1s` (50% headroom for timer jitter, plus a 1-second `SEEK_SLACK`); only a jump beyond that is a seek. In [external audio mode](/player/external-mode/) there is no `<audio>` element to read the rate from, so 1x is assumed.
...
From CommonJS, the singleton is the `default` export: `require('@arraypress/waveform-tracker').default` (1.1.0+ — earlier versions returned `{}`). The package's `exports` map sends `import` to the ESM build and `require` to the CJS build; `./dist/*`, `./src/*` and `./package.json` stay reachable for direct paths.
...
```js
// Call this as early as players may appear.
...
:::note[Calling `init()` again]
A second `init()` **reconfigures** the tracker in place (1.1.0+): the new config applies to players already being tracked, the session id is kept (it's the same page session), and no second set of `document` listeners is added. `reset()` is the full teardown — it untracks every player, removes the `document` `ready`/`destroy` listeners, and clears the config and session id, so players created afterwards are ignored until the next `init()`, which starts a new session id.
:::
