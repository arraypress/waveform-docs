...
During playback the player already drives this for you — it highlights the
marker the playhead has most recently passed (the one with the greatest `time` ≤
the current time) and reveals its label, in both audio modes. Call
`setActiveMarker()` only when an external controller wants to override which
