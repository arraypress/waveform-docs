...
Swaps the current track at runtime. Pauses, resets the audio / error / progress state, **resets per-track options** (`markers` → `[]` and `waveform` → `null`), merges the supplied `options`, **reconciles the artist and artwork DOM in place** — adding, updating, or removing each to match the new track — then calls `load()`. It auto-plays the new track unless `options.autoplay === false`.
...
| `options` | `object` | Optional. Per-track overrides merged over current options (e.g. `markers`, `waveform`, `artwork`, `artworkAlt`, `bpm`, `autoplay`). |
