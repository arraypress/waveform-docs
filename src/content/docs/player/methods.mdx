...
<Aside type="tip">
You rarely need `new` for declarative markup. Any element carrying `data-waveform-player` is instantiated automatically on `DOMContentLoaded`, and you can re-scan the DOM at any time with [`WaveformPlayer.init()`](#waveformplayerinitroot). See [Installation](/getting-started/installation/).
</Aside>
...
Pauses playback. Self mode calls `audio.pause()`; external mode dispatches a cancelable `waveformplayer:request-pause`. If a listener calls `preventDefault()`, the player stays [`currentlyPlaying`](#static-properties) — the same bookkeeping a vetoed `request-play` skips (1.28.0+; earlier versions cleared it before dispatching). **Returns** `void`.
...
| Parameter | Type | Description |
| --- | --- | --- |
| `seconds` | `number` | Target time; clamped to `[0, duration]`. A non-finite value is ignored (1.28.0+). |
...
Loads (or reloads) audio into the existing player: sets the title, fetches or generates and draws the peaks, renders markers, initializes Media Session (self mode), and fires the `onLoad` callback — after a `.json` peaks sidecar has been applied (1.28.0+). Self mode awaits the audio's `loadedmetadata` event. Any load, decode or audio error funnels through `onError`, and a fresh `load()` clears a previous error state, so it doubles as a retry (1.28.0+).

A `load()` superseded by a newer `load()` / `loadTrack()` (or by `destroy()`) stops at its next `await`, so a slow decode can never land on the track that replaced it (1.28.0+).
...
Swaps the current track at runtime. Pauses, resets the audio / error / progress state and the time readout, **resets per-track options** (`markers` → `[]`, `waveform` → `null`, and — 1.28.0+ — `bpm` and `album`, unless the call supplies them), merges the supplied `options`, **reconciles the artist and artwork DOM in place** — adding, updating, or removing each to match the new track — then calls `load()`. It auto-plays the new track unless `options.autoplay === false`.

| Parameter | Type | Description |
| --- | --- | --- |
| `url` | `string` | New audio URL. |
| `title` | `string \| null` | Optional. Track title; `null` keeps the current title; `''` derives it from the URL filename. |
| `artist` | `string \| null` | Optional. Any string adds or updates the artist row; `''` removes it; `null` keeps the current one. |
| `options` | `object` | Optional. Per-track overrides merged over current options (e.g. `markers`, `waveform`, `artwork`, `artworkAlt`, `bpm`, `album`, `preload`, `autoplay`). |
...
<Aside type="note" title="Artist & artwork reconcile in place (1.21.0+)">
`loadTrack` reconciles the info-row metadata against the new track, so you can swap tracks whose metadata *presence* differs without rebuilding the player:

- **`artist`** (3rd argument) — a string adds or updates the `.waveform-artist` row, `''` removes it, `null` leaves the current one untouched.
- **`options.artwork`** — a URL adds or updates the `.waveform-artwork` image; `null` (or `''`) removes it; omit the key to leave existing artwork as-is.
- **`options.artworkAlt`** — sets the artwork's `alt` text. Pass it alongside `artwork`, or on its own to relabel the current image.

Only the fields you pass are touched — omitted metadata is preserved. Before 1.21.0, `loadTrack` could only update artist/artwork that *already existed*; adding or removing them needed a full teardown.
</Aside>

<Aside type="caution" title="Why the per-track reset matters">
`mergeOptions` skips `null` / `undefined`, so a later call can't clear an earlier value by passing `null`. That is exactly why `loadTrack` explicitly resets `options.waveform = null` and `options.markers = []` for every track — otherwise the previous track's peaks and markers would persist and the new track would redraw the *old* waveform (the 1.8.1 fix). Since 1.28.0 `bpm` and `album` get the same treatment, so a tempo badge or lock-screen album no longer carries over to the next track. If you add your own per-track options, reset them here too.
</Aside>
...
| Parameter | Type | Description |
| --- | --- | --- |
| `rate` | `number` | Clamped to `0.25`–`4` — the range browsers keep audible. Non-numeric values are ignored. |

Applies to the audio, persists to `options.playbackRate`, and refreshes the speed menu UI. The rate also survives later loads — a `loadTrack()` no longer drops back to 1× (1.28.0+). No-op in external mode. **Returns** `void`.
...
| Parameter | Type | Description |
| --- | --- | --- |
| `data` | `number[] \| string` | Inline array, comma-separated string, JSON-array string, or a `.json` URL — a `?query` / `#hash` after `.json` is fine — fetched asynchronously; embedded `{ peaks, markers, bpm }` honoured. |

Malformed input normalizes to `[]`. **Returns** `Promise<boolean>` for a `.json` URL — whether the sidecar was applied; `false` on a network error, a non-2xx response or a file with no peaks, leaving the canvas as it was — and `void` for inline peaks (1.28.0+; earlier versions returned nothing either way).
...
Stores the external duration, emits `waveformplayer:timeupdate`, and synthesizes a one-shot `waveformplayer:ended` once `currentTime / duration >= 1`. Since 1.28.0 it also places the markers whenever the duration changes (no manual re-render needed), and it leaves the playhead alone while the user is dragging the scrubber. **Returns** `void`.
...
Tears the player down completely. It flags itself as destroying, emits `waveformplayer:destroy` (first, so listeners can release references), cancels any in-flight load, stops the animation frame loop, aborts all DOM/document listeners via its `AbortController`, disconnects the `ResizeObserver`, removes the resize handler, drops itself from `instances` and `currentlyPlaying`, pauses, resets and nulls the audio element, and empties the container. **Returns** `void`.

Since 1.28.0 `destroy()` doesn't go through `pause()`, so in external mode it no longer emits `waveformplayer:request-pause` — a wrapper remounting an inline player won't pause the controller's (e.g. `waveform-bar`'s) playback.
...
Re-scans for `[data-waveform-player]` elements and instantiates any that aren't yet initialized. Idempotent: the library marks each handled element with `data-waveform-initialized="true"`, so re-running skips them. It also runs automatically on `DOMContentLoaded`. **Returns** `void`.

| Parameter | Type | Default | Description |
| --- | --- | --- | --- |
| `root` | `Document \| Element` | `document` | Subtree to scan. Matched against itself as well as its descendants, so passing a single player element works. |
...
Pass a root to initialise only the part of the page you control — the usual pairing with either opt-out, the [`data-waveform-autoinit="false"` attribute](/player/data-attributes/#initialising-only-what-you-control) or the [`/no-autoinit` entry point](/player/data-attributes/#if-you-dont-control-the-page-either):

```js
WaveformPlayer.init( document.querySelector( '#my-app' ) );
```

Scanning this way keeps the idempotency flagging and per-element error isolation, so one element that fails to construct never takes the rest of the subtree down with it — worth preferring over a hand-rolled `querySelectorAll` loop.

<Aside type="tip" title="Added in 1.26.0">
The `root` parameter. Calling `init()` with no argument scans the whole document exactly as before.
</Aside>
...
**Returns** `string | undefined` — the `.json` URL, or `undefined` for `null` / `undefined` / empty input or an unrecognised extension. Pass the result as the `waveform` option to skip the Web Audio decode entirely.
...
| `samples` | `number` | Optional, default `1800`. Peak resolution. |
...
| `setWaveformData(data)` | both | `Promise<boolean> \| void` |
...
| `WaveformPlayer.init(root?)` | static | `void` |
