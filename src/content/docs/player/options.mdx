...
Here is a fully-loaded player — artwork, title, subtitle, and live BPM — to anchor the options below:
...
| `samples` | `256` | `number` | Source peak resolution decoded; resampled to fit the visible bars (fidelity headroom, **not** bar count). `data-samples`. |
...
External players have no `<audio>` element — `seekTo`, `seekToPercent`, `setVolume`, `setPlaybackRate`, and Media Session are no-ops. Instead you drive visuals with `setPlayingState()` / `setProgress()`, and `play`/`pause`/seek dispatch **cancelable** `waveformplayer:request-*` events. See [Audio modes](/player/audio-modes/).
:::
...
| `waveformColor` | `null` | `string \| string[] \| null` | Unplayed colour; `null` = preset. Array = vertical gradient stops, e.g. `['#fafafa','#71717a']`. `data-waveform-color` (legacy: `data-color`). |
| `progressColor` | `null` | `string \| string[] \| null` | Played-through colour; `null` = preset; array = gradient. `data-progress-color`. |
| `buttonColor` | `null` | `string \| null` | Play-button border + icon colour; `null` = preset. `data-button-color`. |
| `buttonHoverColor` | `null` | `string \| null` | Button hover colour; `null` = preset. `data-button-hover-color`. |
| `textColor` | `null` | `string \| null` | Title colour; `null` = preset. `data-text-color`. |
| `textSecondaryColor` | `null` | `string \| null` | Subtitle / time / BPM colour; `null` = preset. `data-text-secondary-color`. |
| `backgroundColor` | `null` | `string \| null` | Player background token; `null` = preset. `data-background-color`. |
| `borderColor` | `null` | `string \| null` | Player border token; `null` = preset. `data-border-color`. |

### Gradients

`waveformColor` and `progressColor` accept either a single CSS colour string or an array of stops, rendered top-to-bottom as a vertical gradient. A single-element array collapses to one colour. Via `data-*`, pass a JSON array string:
...
Auto-detection (and live re-detection on `<html>`/`<body>` theme flips or OS `prefers-color-scheme` changes) only runs while `colorPreset` is `null` and a colour is left unset. Setting an explicit `colorPreset` **or** any colour opts that out. Call [`refreshTheme()`](/player/api/#refreshtheme) to re-detect manually.
:::
...
| `showInfo` | `true` | `boolean` | Render the info block (artwork/title/subtitle/meta). `data-show-info`. |
| `showTime` | `true` | `boolean` | Render current/total time display. `data-show-time`. |
| `showHoverTime` | `false` | `boolean` | Declared and parsed, but no runtime hover-tooltip exists yet — currently a no-op. `data-show-hover-time`. |
...
| `subtitle` | `null` | `string \| null` | Subtitle row; `''` hides it in `loadTrack`. `data-subtitle`. |
...
When you swap tracks at runtime with [`loadTrack()`](/player/api/#loadtrack), some options are deliberately reset so the new track can't inherit stale state:

- `waveform` &rarr; `null` (otherwise the previous track's peaks would redraw)
- `markers` &rarr; `[]`
- `subtitle` &rarr; passing `''` clears the row
...
| `data-button-color` | `buttonColor` | CSS colour. |
| `data-button-hover-color` | `buttonHoverColor` | CSS colour. |
| `data-text-color` | `textColor` | CSS colour. |
| `data-text-secondary-color` | `textSecondaryColor` | CSS colour. |
| `data-background-color` | `backgroundColor` | CSS colour. |
| `data-border-color` | `borderColor` | CSS colour. |
...
<CardGrid>
  ...
  <Card title="Methods & API" icon="setting">
    Drive players at runtime — [`loadTrack`, `seekTo`, `setProgress`, and the static helpers](/player/api/).
  </Card>
  ...
  <Card title="Audio modes" icon="random">
    `self` vs `external` and how to wire a visualization-only player — see [Audio modes](/player/audio-modes/).
  </Card>
