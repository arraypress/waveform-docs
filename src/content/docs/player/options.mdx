...
A handful of options accept a shorthand. The **canonical name always wins** if both are present. (For `data-color` / `data-theme` that's true from 1.28.0 — earlier versions applied the legacy attribute last, so it won. `data-color` also accepts a JSON gradient stop array from 1.28.0, like `data-waveform-color`.)
...
In plain JS and `data-*`, `style`/`data-style` is the waveform-style alias. In every framework wrapper — Astro, React, Vue and Svelte — `style` stays the framework's CSS-style attribute on the host element and is never forwarded as an option, so there you must use `waveformStyle` for the visual style. (The Vue and Svelte prop types still inherit the core's `style?: WaveformStyle` alias, but at runtime the value lands on the host element.) See [the wrapper docs](/getting-started/installation/).
:::
...
| `preload` | `'metadata'` | `'auto' \| 'metadata' \| 'none'` | `<audio>` preload hint (self mode). With `'none'` the browser fetches nothing until play, so duration and markers only resolve then — pair it with pre-computed peaks so the waveform still paints immediately. `data-preload`. |
| `crossOrigin` | `null` | `'anonymous' \| 'use-credentials' \| null` | CORS mode for the `<audio>` (self mode). Unset by default so the player behaves like a plain `<audio>` and never forces a CORS request — setting it breaks playback of media on origins without `Access-Control-Allow-Origin` (default-config S3/CDN). Set only if you need it. `data-cross-origin`. |
| `audioMode` | `'self'` | `'self' \| 'external'` | `'self'` owns an `<audio>`; `'external'` is visualization-only. `data-audio-mode`. |
| `playbackRate` | `1` | `number` | Initial playback rate (self mode). Kept across loads and `loadTrack()` (1.28.0+; earlier versions dropped back to 1× on the first load). `data-playback-rate`. |
...
| `barWidth` | `2` | `number` | Bar width px; minimum `0.1` (1.28.0+ — smaller values, including `0`, are raised to it). Overridden by per-style defaults when neither option nor `data-*` set it. `data-bar-width`. |
...
| `buttonRadius` | `null` | `number \| string \| null` | **1.22.0+** `null` = stylesheet default (`50%`, a circle); `0` = square; number = px; string (`'0.5rem'`) verbatim. Sets `--wfp-btn-radius`. Shapes the `circle` style — the bare `minimal` glyph has no box to round. `data-button-radius`. |
...
| `playbackRates` | `[0.5, 0.75, 1, 1.25, 1.5, 1.75, 2]` | `number[]` | Selectable rates in the speed menu. Entries outside `0.25`–`4` are dropped, so the menu can never offer a rate [`setPlaybackRate`](/player/methods/#setplaybackraterate) would refuse. `data-playback-rates` (JSON array **or** a bare comma-separated list). |
...
| `showBPM` | `false` | `boolean` | Show the BPM badge — the `bpm` option, else a peaks sidecar's `bpm`, else the detected tempo. The badge stays hidden when the track has none (1.28.0+: silence, pads or speech no longer show a made-up `120`). **`data-show-bpm`** (the data key differs from the option key). |
...
| `artworkAlt` | `'Album artwork'` | `string` | Alt text for the artwork image. Localize for non-English UIs. Info-row placement only — button artwork is decorative. `data-artwork-alt`. |
| `artworkPosition` | `'info'` | `'info' \| 'button'` | **1.22.0+** Where `artwork` renders. `'info'` = the info row beside the title (needs `showInfo`). `'button'` = the cover becomes the play button, and still works with the info row hidden. Only one placement ever renders, so a cover can't appear twice. `data-artwork-position`. |
| `album` | `''` | `string` | Media Session album metadata only. Per-track: `loadTrack()` resets it unless the call passes one (1.28.0+). `data-album`. |
| `unknownTrackText` | `'Unknown Track'` | `string` | Media Session (lock-screen) title fallback used when no `title` is set. Localize for non-English UIs. `data-unknown-track-text`. |
| `bpm` | `null` | `number \| null` | Known BPM; wins over a sidecar's `bpm` and auto-detection (shows even when peaks are pre-supplied). Per-track: `loadTrack()` resets it unless the call passes one (1.28.0+). `data-bpm`. |
...
| `seekValueText` | `null` | `string \| null` | Template for the slider's spoken `aria-valuetext`; `%1$s` is the current time, `%2$s` the total (both `M:SS`). Sequential `%s` and reordered positional args supported. Falls back to `'%1$s of %2$s'`. `data-seek-value-text`. |
| `playPauseLabel` | `'Play/Pause'` | `string` | Play button `aria-label` (screen-reader name). Localize for non-English UIs. `data-play-pause-label`. |
| `speedLabel` | `'Playback speed'` | `string` | Speed button + menu `aria-label`. Localize for non-English UIs. `data-speed-label`. |
...
Most option callbacks have an event-dispatch twin (see [Events](/player/events/)) that fires for the same moment. `onLoad`, `onError`, `onNextTrack` and `onPreviousTrack` have none.

| Option | Type | Fires |
| --- | --- | --- |
| `onLoad` | `(player) => void` | After a track's waveform is drawn — once a `.json` peaks sidecar has been applied (1.28.0+). |
...
| `onTimeUpdate` | `(currentTime, duration, player) => void` | On progress; same arg order in both modes. Keeps firing in a background tab (1.28.0+). |
| `onNextTrack` | `(player) => void` | Media Session "next track" control (lock screen, headset). Setting it shows that button. |
| `onPreviousTrack` | `(player) => void` | Media Session "previous track" control. Setting it shows that button. |
...
- `markers` &rarr; `[]`
- `bpm` &rarr; `null` and `album` &rarr; `''` unless the call passes them (1.28.0+ — the previous track's tempo badge and lock-screen album used to carry over)
- `title` &rarr; `null` keeps the current title; `''` derives it from the URL
- `artist` &rarr; passing `''` clears the row

:::caution[`mergeOptions` skips `null`]
The internal merge ignores `null`/`undefined`, so a later source can't clear an earlier value by passing `null`. That's exactly why `loadTrack` explicitly resets `waveform`, `markers`, `bpm` and `album` per track. If you add your own per-track options, reset them the same way.
:::
...
| `data-cross-origin` | `crossOrigin` | string (`anonymous` / `use-credentials`). |
...
| `data-waveform-gradient` | `waveformGradient` | string (`vertical` \| `horizontal` \| `diagonal`). |
...
| `data-button-radius` | `buttonRadius` | bare number &rarr; px float; unit string (`'0.5rem'`) verbatim. `"0"` is read, not skipped. |
| `data-button-align` | `buttonAlign` | string. |
| `data-show-playback-speed` | `showPlaybackSpeed` | bool. |
| `data-playback-rates` | `playbackRates` | JSON array (`[0.5,1,2]`) or a bare list (`0.5, 1, 2`). |
| `data-color-preset` / `data-theme` | `colorPreset` | `data-color-preset` wins (1.28.0+); `data-theme` is legacy. |
| `data-waveform-color` / `data-color` | `waveformColor` | CSS colour or JSON gradient array (either name, 1.28.0+); `data-waveform-color` wins (1.28.0+); `data-color` is legacy. |
...
| `data-seek-value-text` | `seekValueText` | string (printf template). |
| `data-play-pause-label` | `playPauseLabel` | string. |
| `data-speed-label` | `speedLabel` | string. |
| `data-title` | `title` | string. |
| `data-artist` | `artist` | string. |
| `data-album` | `album` | string. |
| `data-artwork` | `artwork` | string. |
| `data-artwork-alt` | `artworkAlt` | string. |
| `data-artwork-position` | `artworkPosition` | string (`info` \| `button`). |
| `data-unknown-track-text` | `unknownTrackText` | string. |
