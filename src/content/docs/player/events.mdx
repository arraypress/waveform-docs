...
Note the asymmetry: `onLoad` / `onError` (and the Media Session
`onNextTrack` / `onPreviousTrack`) have **no** DOM-event twin, and
...
| `onPause` | `(player) => void` | Playback pauses — both modes. Runs once when a track ends, before `onEnd` (1.28.0+; self mode used to run it twice there). |
...
| `onNextTrack` | `(player) => void` | The Media Session "next track" control is used (lock screen, headset). Setting it lights up that button. |
| `onPreviousTrack` | `(player) => void` | The Media Session "previous track" control is used. Setting it lights up that button. |
...
In self mode `timeupdate` (and `onTimeUpdate`) is emitted once per animation
frame. Browsers stop the frame loop in background tabs while the audio plays on,
so since 1.28.0 the native `<audio>` `timeupdate` takes over whenever the loop
has been quiet for 500ms — progress keeps flowing (at the browser's slower rate)
in a hidden tab. A self-mode track's end also emits exactly one
...
aborted. It does **not** emit `request-pause` (1.28.0+), so tearing down an
external-mode player never pauses its controller.
...
- `request-play` / `request-pause`: `preventDefault()` tells the player the
  request was refused, so it leaves the `singlePlay` bookkeeping alone — a vetoed
  `request-play` doesn't claim `WaveformPlayer.currentlyPlaying`, and a vetoed
  `request-pause` keeps it (1.28.0+; before that, `request-pause` cleared it
  regardless). Neither event flips the play/pause visual, vetoed or not — that
  only changes when your controller calls `setPlayingState()`.
