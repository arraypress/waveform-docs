...
### Initialising only what you control

<Aside type="tip" title="Added in 1.26.0">
Set `data-waveform-autoinit="false"` on the document element to switch the automatic scan off entirely.
</Aside>

If your page renders markup you did not author — user-submitted content, a CMS, an editor canvas — you may not want *every* `[data-waveform-player]` on the page to become a player. Opt the whole document out:

```html
<html data-waveform-autoinit="false">
```

Nothing is initialised until you ask for it. Pass `init()` a root to scan only the subtree you own:
...
The root is matched as well as its descendants, so handing `init()` a single player element works too. Constructing players directly with `new WaveformPlayer( el )` also still works — only the scan on import is suppressed.

The attribute goes on `<html>` rather than a JS global because ES module imports are evaluated *before* the importing module's body runs, so a consumer with a static `import` never gets the chance to set a flag first. Markup is in place before any script executes.

### If you don't control the page either

<Aside type="tip" title="Added in 1.27.0">
`@arraypress/waveform-player/no-autoinit` is the same library with no import-time scan at all.
</Aside>

The attribute above assumes you own the document element. Plenty of consumers don't: a CMS block, a plugin, a widget dropped into someone else's template. Your bundle is evaluated inside a page whose `<html>` belongs to the host theme — and in an editor, to an iframe you don't own either. There is nothing to mark.

Import the library without the scan instead:
...
Same class, same options, same events, same `window.WaveformPlayer`. Ships as ESM and CJS with its own types. The only thing it drops is the document-wide scan that the default entry point runs when it loads.

Use the attribute if you render the page; use this entry point if you're bundled into one.

:::caution
Load **one entry point per bundle**. Each carries its own copy of the class and its own instance registry, so importing both gives you two libraries that can't see each other's players — the same way the IIFE and ESM builds always have.
:::

:::caution
This scopes **initialisation**, not sanitisation. [`playIcon` / `pauseIcon`](/player/options/) (and their `data-play-icon` / `data-pause-icon` equivalents) inject raw markup by design, for any player you *do* build. Untrusted content still needs sanitising before it reaches the page.
:::
...
| **JSON** | `data-markers` takes a JSON **array**. Malformed JSON — and valid JSON that isn't an array, such as `2` or `{"a":1}` — is **warned about and skipped** (the attribute is ignored, never thrown). `data-playback-rates` additionally accepts a bare comma-separated list (`0.5, 1, 2`). |
| **Color** | `data-waveform-color` / `data-progress-color` (and, 1.28.0+, the legacy `data-color`) accept a CSS color string **or** a JSON array of gradient stops. A value starting with `[` is parsed as JSON; on parse failure it falls back to the raw string. |

Since **1.25.0**, every option is normalized after parsing, whichever path it
arrived by — attribute, constructor, or framework wrapper. A value the player
can't use resolves to its documented default and warns, naming the option,
rather than reaching the code that consumes it:

- **Numbers** that don't parse (`data-height="tall"`) fall back to the default;
  values outside a sane range are clamped.
- **Enumerated options** (`layout`, `buttonStyle`, `waveformStyle`, `preload`,
  `crossOrigin`, …) fall back when the value isn't one of the documented ones.
- **Lists** (`markers`, `playbackRates`) must be lists; individual entries that
  aren't usable — a marker with no numeric `time`, a rate outside `0.25`–`4` —
  are dropped rather than rendered broken.

So a typo costs you that one option, never the player.
...
| `data-cross-origin` | `crossOrigin` | `anonymous` \| `use-credentials` | — (unset) |
...
| `data-button-radius` | `buttonRadius` | int (px) **or** unit string (`0.5rem`) | `null` |

`data-button-size` and `data-button-radius` share one rule: a bare number (`data-button-size="64"`) is treated as pixels; a value with a unit (`data-button-radius="0.5rem"`) is kept verbatim. They set `--wfp-btn-size` and `--wfp-btn-radius`.

`data-button-radius="0"` squares the button — the `0` is read, not skipped as an empty value. Unset leaves the stylesheet's circular `50%`.
...
| `data-color` | `waveformColor` *(legacy alias)* | CSS color **or** JSON gradient array (1.28.0+) | — |
...
| `data-artwork-alt` | `artworkAlt` | string (artwork image alt text — info-row placement only) | `Album artwork` |
| `data-artwork-position` | `artworkPosition` | `info` \| `button` | `info` |
| `data-album` | `album` | string (Media Session metadata only) | `''` |
| `data-unknown-track-text` | `unknownTrackText` | string (Media Session title fallback when no `title`) | `Unknown Track` |
...
| `data-seek-value-text` | `seekValueText` | string (spoken `aria-valuetext` template; `%1$s` current, `%2$s` total) | `null` |
| `data-play-pause-label` | `playPauseLabel` | string (play button `aria-label`) | `Play/Pause` |
| `data-speed-label` | `speedLabel` | string (speed button + menu `aria-label`) | `Playback speed` |
...
| `data-color` | `data-waveform-color` | Legacy. Accepts a JSON gradient array too (1.28.0+). |
| `data-theme` | `data-color-preset` | Legacy. |

<Aside type="note" title="Legacy colour aliases (1.28.0+)">
Before 1.28.0, `data-color` and `data-theme` were applied *after* their canonical attributes, so on an element carrying both the legacy name won. From 1.28.0 they follow the same rule as `data-src` / `data-style`: the canonical attribute wins.
</Aside>
