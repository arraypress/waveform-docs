...
The attribute goes on `<html>` rather than a JS global because ES module imports are evaluated *before* the importing module's body runs, so a bundled consumer never gets the chance to set a flag first. Markup is in place before any script executes.
