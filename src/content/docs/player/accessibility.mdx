...
The library ships **no** built-in translations — that keeps the core lean and lets your own i18n pipeline (WordPress text domains, `react-i18next`, etc.) own the strings. But if you just want a drop-in, here are ready-made objects for a few common languages. Spread one into your options (or emit the matching `data-*` attributes).
...
<Aside type="note">
Treat these as a starting point, not gospel — have a native speaker or your normal translation workflow confirm them before shipping. Keep the `%1$s` / `%2$s` placeholders intact in `seekValueText`; only the surrounding words should change.
</Aside>
