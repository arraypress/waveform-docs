...
:::note[Requires core 1.27.0 or newer]
The component loads the core library's [`/no-autoinit` entry point](/player/data-attributes/#if-you-dont-control-the-page-either), so mounting it never scans the page for markup it doesn't own. That subpath was added in `@arraypress/waveform-player` 1.27.0 — if you upgrade the wrapper on its own, bump the core alongside it.
:::
...
| `setPlaybackRate` | `(rate: number) => void` | Clamped `0.25..4`. Self mode only. |
