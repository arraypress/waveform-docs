...
The `.json` may be a bare peaks array or an object with embedded `{ peaks, markers }`. See [pre-computed peaks](/player/peaks/) for generating the file (including `WaveformPlayer.getPeaksUrl()` to derive the sibling URL).
...
<CardGrid>
	...
	<Card title="Pre-computed peaks" icon="approve-check">
		Generate and serve `.json` peaks to skip decoding. [Peaks](/player/peaks/)
	</Card>
