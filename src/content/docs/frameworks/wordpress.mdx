...
The fastest path — point WordPress at jsDelivr or unpkg. These use `@latest` for the current release; pin an exact version (`@1.23.0`) instead for reproducible builds.
...
add_action( 'wp_enqueue_scripts', function () {
	wp_enqueue_style(
		'waveform-player',
		'https://cdn.jsdelivr.net/npm/@arraypress/waveform-player@latest/dist/waveform-player.css',
		array(),
		'1.14.0'
	);
	...
	wp_enqueue_script(
		'waveform-player',
		'https://cdn.jsdelivr.net/npm/@arraypress/waveform-player@latest/dist/waveform-player.min.js',
...
Enqueue conditionally to keep pages lean — for example `if ( is_singular() && has_shortcode( get_post()->post_content ?? '', 'waveform' ) ) { … }`, or gate on a page template. The whole player is ~12 KB of JS gzipped plus the stylesheet.
</Aside>
...
### Bundled into a block (`@wordpress/scripts`)

The two routes above enqueue the browser build as a classic script, which self-initialises — right for a theme, where the page is yours.

A **block** is a different situation. Your JavaScript is bundled and runs inside a page built from post content you didn't author, and in the editor inside an iframe you don't own. Importing the package root there would scan that whole document and upgrade every `[data-waveform-player]` it found, including markup belonging to other plugins. Import the non-scanning entry point instead:

```js
import WaveformPlayer from '@arraypress/waveform-player/no-autoinit';

// Build your own container, then mount exactly that one:
const container = document.createElement( 'div' );
container.setAttribute( 'data-waveform-player', '' );
container.setAttribute( 'data-url', url );
blockRoot.appendChild( container );

new WaveformPlayer( container );
```

Constructing the container yourself has a second benefit: the player only ever reads `data-*` attributes you set, so nothing an author typed into post content can reach the option surface.

<Aside type="note">
This is how WordPress core does it. The Playlist block added in [7.1](/news/wordpress-7-1/) bundles the library and mounts its own containers rather than letting the page scan. Requires `@arraypress/waveform-player` 1.27.0 or newer — see [choosing an entry point](/player/data-attributes/#if-you-dont-control-the-page-either).
</Aside>
...
- **Enqueuing in a theme vs. bundling in a block.** An enqueued classic script scanning the page is the point — that's how the markup in post content becomes players. Inside a block bundle it is a liability, because the document isn't yours. Use [`/no-autoinit`](/player/data-attributes/#if-you-dont-control-the-page-either) there. If you need to suppress the scan on a page you *do* control, `data-waveform-autoinit="false"` on `<html>` (via the `language_attributes` filter) does it without changing your import.
