...
<Aside type="caution">
For production, pin an **exact** version (for example `@1.22.0`) rather than `@1`. A floating major range means a CDN-cached patch can change behaviour without a deploy on your side.
</Aside>
...
```bash
curl -O https://cdn.jsdelivr.net/npm/@arraypress/waveform-player@1.22.0/dist/waveform-player.min.js
curl -O https://cdn.jsdelivr.net/npm/@arraypress/waveform-player@1.22.0/dist/waveform-player.css
```
